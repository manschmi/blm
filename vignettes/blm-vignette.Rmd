...
Additional terms, as already seen above.
```{r}
w0 <- .2
w1 <- 3
w2 <- 10
...
##Some Examples for Behaviour of Bayesian Linear Models 
...
b <- 0.001
y <- w0 + w1*x + rnorm(length(x), mean=0, sd=sqrt(1/b) )
blm_mod <- blm(y~x, beta = b)
coefs <- coef(blm_mod)
covars <- covar.blm(blm_mod)

y0 <- w0 + w1*x
test <- function(...){
yn <-  y0 + rnorm(length(x), mean=0, sd=sqrt(1/b) )
coef(lm(yn~x))
}
tests <- lapply(1:10000, test)

intercepts <- sapply(tests, function(t) t[1])
slopes <- sapply(tests, function(t) t[2])

par(mfrow=c(1,2))
kernel_density(blm_mod$posterior, 
               xlim=c(1.5,2.5), ylim=c(-4,4), 
               main = 'posterior of blm fit')
heatscatter(slopes, intercepts, 
            xlim=c(1.5,2.5), ylim=c(-4,4) ,
            main = 'sampling of lm fit with random error of precision')
par(mfrow=c(1,1))
